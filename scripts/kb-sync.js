...
function git(dir, args, quiet = false) {
  return execFileSync("git", ["-C", dir, ...args],
    { encoding: "utf8", stdio: ["ignore", "pipe", quiet ? "ignore" : "pipe"] }).trim();
}
function gitTry(dir, args) { try { return { ok: true, out: git(dir, args, true) }; }
...
let dataDir;
try { dataDir = (JSON.parse(fs.readFileSync(configPath, "utf8")).data_dir || "").replace(/^~(?=$|\/)/, os.homedir()); }
catch { die(`ERROR: cannot read ${configPath} (run /kb once to set data_dir)`, 3); }
...
if (setRemote !== null) {
  ...
  if (!push.ok) die(`ERROR: remote added but push failed: ${push.out}\nThe remote is set; fix access/URL and run /kb sync.`, 5);
...
const ahead = parseInt(gitTry(dataDir, ["rev-list", "--count", "@{u}..HEAD"]).out || "0", 10) || 0;
const push = gitTry(dataDir, ["push"]);
if (!push.ok) die(`ERROR: pull ok, push failed: ${push.out}`, 6);

// --- report ---
console.log("SYNCED");
console.log(`pull: ${/up to date/i.test(pull.out) ? "already up to date" : (pull.out.split("\n").slice(-1)[0] || "updated")}`);
console.log(`push: ${ahead === 0 ? "nothing to push" : `pushed ${ahead} commit${ahead === 1 ? "" : "s"}`}`);
