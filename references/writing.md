...
1. **`--file <path>`** — the file you just `Write`/`Edit`-ed. This is the primary
   path: `${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-save.js --file entries/<name>.md ...`.
...
3. **stdin (heredoc)** — the fallback, for piping a program's output straight in
   (e.g. `some-extractor | ${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-save.js --slug ...`).
   Empty stdin (a TTY or `< /dev/null`) does NOT count as stdin content.

## Gotchas (read before saving)

- **Command must start with the launcher** — the allowed-tools pattern is `Bash(${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-save.js *)`. Using `cat entry.md | node kb-save.js` won't match and the call will be blocked; pass `--file` (or a bare heredoc that begins with the launcher) instead.
...
1. **Find candidate link targets.** To propose `links`, you need the ids of
   ...
   knowledge (`${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-search.js "<term>" ...`) and
...
3. **Save immediately** — hand the draft file to the helper with `--file`:
   `${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-save.js --slug "<slug-from-title>" --file entries/<slug>.md`
   ...
   instead of `--file` — the command must still start with the launcher.)
...
3. **Save immediately** — commit the edited file:
   `${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-save.js --edit <id> [--slug "<new-slug>"]`
...
`${CLAUDE_SKILL_DIR}/scripts/run-node.sh ${CLAUDE_SKILL_DIR}/scripts/kb-save.js --set-remote "<url>"`
